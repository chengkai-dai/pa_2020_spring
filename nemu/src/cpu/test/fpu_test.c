...
void fpu_test_mul()
{
	...
	for (i = 0; i < sizeof(input) / sizeof(float); i++)
	{
		for (j = 0; j < sizeof(input) / sizeof(float); j++)
		{
			...
			printf("a       sign %x, exp %x, %d\n", a.sign, a.exponent, a.exponent);
			printf("b       sign %x, exp %x, %d\n", b.sign, b.exponent, b.exponent);
			printf("res     sign %x, exp %x, %d\n", res.sign, res.exponent, res.exponent);
			printf("res_fpu sign %x, exp %x, %d\n", res_fpu.sign, res_fpu.exponent, res_fpu.exponent);
			}
			assert(res_fpu.val == res.val);
		}
	}
